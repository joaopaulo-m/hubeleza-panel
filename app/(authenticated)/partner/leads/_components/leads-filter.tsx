...
import { DashboardTreatmentSelector } from './dashboard-treatment-selector';
...
export function LeadsFilters({ treatments }: LeadsFiltersProps) {
  ...
  const hasFilters = startDate || endDate || nameQuery || selectedTreatments.length > 0;
  const activeFiltersCount = [startDate, endDate, nameQuery, selectedTreatments.length > 0].filter(Boolean).length;
